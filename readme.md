...
Este documento fornece uma visão geral abrangente da arquitetura do sistema **Fluxo de Caixa** — plataforma de controle financeiro para comerciantes. Detalha os componentes, integrações, padrões adotados, decisões de design e roadmap de evolução.

O documento segue o modelo **C4** (Simon Brown) para descrição arquitetural em múltiplos níveis de abstração, complementado por diagramas UML de sequência e diagramas de deployment.
...
| Ator | Tipo | Interação |
|---|---|---|
| Comerciante | Pessoa (primário) | Registra lançamentos e consulta consolidado via API |
...
|---|---|---|---|
| API Gateway | .NET 8 + YARP 2.3 | 5000 | Roteamento, ponto único de entrada, Swagger agregado |
...
| Consumer DLQ | .NET 8 BackgroundService + RabbitMQ.Client + Dapper | — | Segunda tentativa de persistência; sem requeue |
...
| Restrição | Descrição |
|---|---|
| **Stack tecnológica** | .NET 8, SQL Server, RabbitMQ, Redis — definidos pelo cliente |
| **Banco relacional obrigatório** | SQL Server como repositório de lançamentos |
...
|---|---|---|---|
| **API Gateway** | `FluxoDeCaixa.Gateway` | 5000 | Reverse proxy YARP; ponto único de entrada |
...
**ADR-003 — Dapper:** Micro-ORM próximo a ADO.NET. SQL explícito revisável por DBA. Performance adequada para 50 req/s com tabela pré-agregada.

**ADR-007 — RabbitMQ (novo):** Pipeline assíncrono decouples escrita do banco. Endpoint responde imediatamente após publish. DLQ garante segunda tentativa de persistência. `RequeueOnError=false` no DLQ evita loop infinito.

**ADR-008 — Redis (novo):** Cache-on-First-Hit com TTL inteligente. Períodos passados: TTL 365 dias. Período atual: TTL até meia-noite. Elimina >99% das queries ao SQL no endpoint de relatório.

**ADR-009 — Testcontainers (novo):** Camadas de teste: unitários (NSubstitute) + integração com containers efêmeros (SQL Server e RabbitMQ). CI/CD executa `dotnet test` sem dependências externas.
